...
python ../common/generate_config.py ${SEED} out/${UNIQUE_NAME}.cluster \
    out/${UNIQUE_NAME}.route results/${UNIQUE_NAME}_mimic_homa --numClusters ${CLUSTERS} \
    --numToRsAndUplinks ${DEGREE} --numServers ${SERVERS} \
    --mimic_homa ${PARALLEL_FLAG} 
...
python ../common/generate_routes.py out/${UNIQUE_NAME}.route \
    ...
    --numToRsAndUplinks ${DEGREE} --mimic_homa ${PARALLEL_FLAG}
# the .cluster file will specify route files

# Configure the queue type
...
mkdir -p results/${UNIQUE_NAME}_mimic_homa
if [[ "${VARIANT}" == "RecordEval" ]]; then
    mkdir -p results/${UNIQUE_NAME}_mimic_homa/eval${CLUSTERS}
fi
if [[ "${VARIANT}" == "RecordAll" ]]; then
    mkdir -p results/${UNIQUE_NAME}_mimic_homa/eval${CLUSTERS}
    mkdir -p results/${UNIQUE_NAME}_mimic_homa/debug${CLUSTERS}
fi
...
if [[ "${PARALLEL_FLAG}" == "--parallel" ]]; then
    ...
    export HOST=`uname -n`
    EXE="mpirun --mca orte_base_help_aggregate 0 -mca btl ^openib -np $(( 1 + ${CLUSTERS} )) ./simulate_mimic_homa"
else
    sed -i "s/<PARALLEL_CONFIG>//" out/${UNIQUE_NAME}.ini
    EXE="./simulate_mimic_homa"
fi
...
# Run Simulation
echo -e "\e[34mRunning simulate_mimic_homa\e[0m"
...
echo "results/${UNIQUE_NAME}_mimic_homa"
