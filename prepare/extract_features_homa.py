...
def find_match(pkt, unmatched_packets, forward_tester, reverse_tester,
    ...
    # print("Did not find match. Packet drop?", pkt)
    return None, None
...
def extract_mimic_features(pkt, second_pkt, emulated_ip, other_ip,
    ...
    ####     Destination ToR, priority, length, time since last packet, EWMA
    #### Egress features:
    ####     Origin server, egress agg, egress agg interface, origin ToR,
    ####     priority, length, time since last packet, EWMA
    ...
    for line in first_file:
        ...
        if second_time:
            ...
            target_time_file.write(str(pkt.get("time")) + " "
                                   + str(delay) + '\n')
            target_drop_file.write('0' + '\n')
        else:
            delay = MAX_RTT_IN_TRAINING
            target_time_file.write(str(pkt.get("time")) + " "
                                   + str(delay) + '\n')
            target_drop_file.write('1' + '\n')
