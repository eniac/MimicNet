...
void EnvirBase::parsimSendState(cState *state,int destProcId)
{
#ifdef WITH_PARSIM

    cCommBuffer *buffer = parsimcomm->createCommBuffer();
    buffer->packObject((cObject *)state);
    parsimcomm->send(buffer, TAG_PARALLELINITIALIZATION, destProcId);
    parsimcomm->recycleCommBuffer(buffer);

    delete state;

#endif
}
cState* EnvirBase::parsimReceiveStateBlocking(int srcProcId)
{
#ifdef WITH_PARSIM
    cCommBuffer *buffer = parsimcomm->createCommBuffer();

    int receivedTag, sourceProcId;
    if(!parsimcomm->receiveBlocking(TAG_PARALLELINITIALIZATION, buffer, receivedTag, sourceProcId))
    {
        parsimcomm->recycleCommBuffer(buffer);
        throw cRuntimeError("Reading was interrupted by user");
        return NULL;
    }


    cObject *stateObj = buffer->unpackObject();

    parsimcomm->recycleCommBuffer(buffer);

    return check_and_cast<cState*>(stateObj);
...
bool EnvirBase::isModuleLocal(cModule *parentmod, const char *modname, int index)
{
    ...
    if (procIds.empty())
    {
        ...
        if(parentmod->isVector())
        	return isModuleLocal(parentmod->getParentModule(), parentmod->getName(), parentmod->getIndex());
        else
        	return isModuleLocal(parentmod->getParentModule(), parentmod->getName(), -1);
    }
...
void EnvirBase::readPerRunOptions()
{
    ...
    num_rngs_per_module = opt->numRNGs;
	createRNGs();
...
void EnvirBase::createRNGs()
{
	...
	for(int i=0;i<num_rngs_per_module;i++)
	{
		...
		rngs.push_back(rng);
        //rngs[pos]->initialize(opt_seedset, pos, num_rngs_per_module, getParsimProcId(), getParsimNumPartitions(), getConfig());
		//same seeds in serial/parallel case
        rngs[pos]->initialize(opt->seedset, pos, num_rngs_per_module, 0, 0, getConfig());
	}
}
