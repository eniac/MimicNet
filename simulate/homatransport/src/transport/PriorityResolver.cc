...
uint16_t
PriorityResolver::getMesgPrio(uint32_t msgSize) const
{
    size_t mid, high, low;
    low = 0;
...
// KN: This function is useful for MimicDCN to assign more accurate priorities
/*uint16_t
PriorityResolver::getSchedPktPrioStaticCBFGraduated(uint32_t bytesToGrant) const
{
    uint32_t bytesToGrantOnWire = HomaPkt::getBytesOnWire(bytesToGrant,
        PktType::SCHED_DATA);                                 
    uint32_t bytesTreatedUnsched = homaConfig->boostTailBytesPrio;
