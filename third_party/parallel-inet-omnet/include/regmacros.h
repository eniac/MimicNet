...
/**
 * Registers a module, which wants to be initialized in parallel.
 *
 * @hideinitializer
 */
#define Register_ParallelInitialization_Module(CLASSNAME) \
  static cObject *__FILEUNIQUENAME__() {cModule *ret = new CLASSNAME; return ret; } \
  EXECUTE_ON_STARTUP(parInitModules.getInstance()->add(new cObjectFactory(opp_typename(typeid(CLASSNAME)),__FILEUNIQUENAME__,NULL,"parInitModules"));)
