...
bool cModule::initializeModules(int stage)
{
    ...
    if (stage < numStages)
    {
        ...
        try {
            if(!simulation.moduleHasRegisterdForParallelInitialization(this->mod_id)) //standard initalization
            {
                initialize(stage);
            }
            else
            {
                cState *preState = simulation.getPreState(this->mod_id,stage); //parallel initalization
                cState &postState = initialize(stage,*preState);
                simulation.setPostState(this->mod_id,&postState,stage);
            }
        } catch (cException&) {
            throw;
