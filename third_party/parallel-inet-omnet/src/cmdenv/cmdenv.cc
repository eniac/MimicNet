...
void Cmdenv::componentInitBegin(cComponent *component, int stage)
{
    if (!opt->expressMode && opt->printEventBanners && component->isEvEnabled())
    {
        ...
        if(component->isModule()){
               ...
               cModule *module = check_and_cast<cModule*>(component);
               bool parallelInitialization = simulation.moduleHasRegisterdForParallelInitialization(module->getId());
               //adjustment of initialization output due to parallel init
               ::fprintf(fout, "{%sInitializing module %s, stage %d\n}",
                       parallelInitialization ? "Parallel " : "", component->getFullPath().c_str(), stage);
           }
           else
           {
               ::fprintf(fout, "{Initializing %s %s, stage %d\n}",
                                        "channel", component->getFullPath().c_str(), stage);
           }
