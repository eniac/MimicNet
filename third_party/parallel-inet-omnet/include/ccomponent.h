...
class SIM_API cComponent : public cDefaultList //implies noncopyable
{
  ...
  protected:
    ...
    /**
     * If a Module registers itself with macro Register_ParallelInitialization_Module for parallel Initialization
     * the omnet core will call this function on every LP with the same state, so that the
     * modules can communicate with the cMessage state.
     */
