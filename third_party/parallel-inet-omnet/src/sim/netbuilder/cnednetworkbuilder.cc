...
void cNEDNetworkBuilder::buildInside(cModule *modp, cNEDDeclaration *decl, bool isPlaceholder)
{
    if (modp->getId() % 50 == 0)
    {
    ...
    // module last.
    submodMap.clear();
    buildRecursively(modp, decl, isPlaceholder);
...
void cNEDNetworkBuilder::buildRecursively(cModule *modp, cNEDDeclaration *decl, bool isPlaceholder)
{
    ...
    if (decl->numExtendsNames() > 0)
    {
        const char *superName = decl->extendsName(0);
        cNEDDeclaration *superDecl = cNEDLoader::getInstance()->getDecl(superName);
        buildRecursively(modp, superDecl, isPlaceholder);
    }

    currentDecl = decl; // switch "context"
    addSubmodulesAndConnections(modp, isPlaceholder);
}

void cNEDNetworkBuilder::addSubmodulesAndConnections(cModule *modp, bool isPlaceholder)
{
