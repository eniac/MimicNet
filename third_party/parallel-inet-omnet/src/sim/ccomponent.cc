...
cState& cComponent::initialize(int stage, cState& state)
{
    throw cRuntimeError("Module `%s' has registered for parallel initialization but it did not implement the initialize(int stage, &cState state) method.",componenttype->getFullName());
    return state;
}
