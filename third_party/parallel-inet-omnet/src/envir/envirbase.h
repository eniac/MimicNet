...
class ENVIR_API EnvirBase : public cRunnableEnvir
{
  ...
  public:
    ...
    virtual void parsimSendState(cState *state,int destProcId); //used by csimulation to send messages between LPs while the parallel initalization
    virtual cState* parsimReceiveStateBlocking(int srcProcId); //used by csimulation to recieve messages from LPs while the parallel initalization
