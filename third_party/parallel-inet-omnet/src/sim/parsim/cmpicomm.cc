...
#define MPI_SEND_BUFFER_PER_PARTITION (512*1024*20)
...
bool cMPICommunications::receiveBlocking(int filtTag, cCommBuffer *buffer, int& receivedTag, int& sourceProcId)
{
    ...
    int ret = MPI_Probe(MPI_ANY_SOURCE, filtTag, MPI_COMM_WORLD, &status);
    ASSERT(ret == MPI_SUCCESS);

    ret = MPI_Get_count(&status, MPI_PACKED, &msgsize);
    ASSERT(ret == MPI_SUCCESS);
    ...
    int err = MPI_Recv(b->getBuffer(), b->getBufferLength(), MPI_PACKED, status.MPI_SOURCE, status.MPI_TAG, MPI_COMM_WORLD, &status);
