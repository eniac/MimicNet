...
for i in range(len(file_list)):
    ...
    if i == 0:
        ...
        tmp_gnuplot_str += ("\"%s\" using 1:2 title \"%s (%.3f)\" w lines lw 5 lc rgb \"%s\",\\\n" % (f+".cdf", title_list[i], w1_distance_list[i - 1], color_list[i]))

gnuplot_str = '''
reset
# set term pdf size 7, 3.85 enhanced dashed color font 'Helvetica,30'
set term pdf size 7, 3.05 enhanced dashed color font 'Helvetica,30'
set output "%s.pdf"
set logscale x
set format x "10^{%%T}"
set nologscale y
