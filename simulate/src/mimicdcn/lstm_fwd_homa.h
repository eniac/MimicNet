...
class MimicNetLstmHoma {
public:
    MimicNetLstmHoma(const int input_size = 20, const int batch_size = 1,
    ...
    void getValue(bool* drop, MimicDType* latency, MimicDType* input) {
        ...
        forward(&prev_drop_, &prev_latency_, data_);

#ifdef DEBUG
        cout.precision(18);
        cout << fixed << prev_drop_ << " " << prev_latency_ << endl;
#endif
    ...
    static unique_ptr<MimicNetLstmHoma> loadFromHdf5(const char* path) {
        ...
        auto model = make_unique<MimicNetLstmHoma>(
        ...
        weight_ih = (MimicDType*)malloc(dataset.getInMemDataSize() * 2);
        dataset.read(weight_ih, H5::PredType::NATIVE_DOUBLE);
        dataset = file.openDataSet("linearD.bias");
        bias_ih = (MimicDType*)malloc(dataset.getInMemDataSize() * 2);
        dataset.read(bias_ih, H5::PredType::NATIVE_DOUBLE);
