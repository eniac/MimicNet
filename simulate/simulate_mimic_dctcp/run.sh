...
USAGE="Usage: ./run.sh Base|RecordEval|RecordAll ingress_model egress_model
                intermimic_model [-s seed] [-r routing] [-l load]
                [-a servers_per_rack] [-b degree (# of ToRs/Aggs/AggUplinks)]
                [-c total_clusters] [-d release|debug] [-k marking_threshold]
                [-p parallel] [-S simulation_length]
...
while getopts "s:r:l:c:a:b:d:p:S:k:mn" opt; do
  case ${opt} in
    s ) # seed
      SEED="$OPTARG"
      ;;
...
python ../common/determine_inter_mimic_intervals.py ${SEED} out/ \
    ...
    --num_servers ${SERVERS} --length $SIMU_LEN --linkSpeed ${LINK_SPEED}
...
python ../common/generate_config.py ${SEED} out/${UNIQUE_NAME}.cluster \
    out/${UNIQUE_NAME}.route results/${UNIQUE_NAME}_mimic_dctcp --numClusters ${CLUSTERS} \
...
mkdir -p results/${UNIQUE_NAME}_mimic_dctcp
if [[ "${VARIANT}" == "RecordEval" ]]; then
    mkdir -p results/${UNIQUE_NAME}_mimic_dctcp/eval${CLUSTERS}
fi
if [[ "${VARIANT}" == "RecordAll" ]]; then
    mkdir -p results/${UNIQUE_NAME}_mimic_dctcp/eval${CLUSTERS}
    mkdir -p results/${UNIQUE_NAME}_mimic_dctcp/debug${CLUSTERS}
fi
...
if [[ "${PARALLEL_FLAG}" == "--parallel" ]]; then
    ...
    export HOST=`uname -n`
    EXE="mpirun --mca orte_base_help_aggregate 0 -mca btl ^openib -np $(( 1 + ${CLUSTERS} )) ./simulate_mimic_dctcp"
else
    sed -i "s/<PARALLEL_CONFIG>//" out/${UNIQUE_NAME}.ini
    EXE="./simulate_mimic_dctcp"
fi
...
# Run Simulation
echo -e "\e[34mRunning simulate_mimic_dctcp\e[0m"
...
echo "results/${UNIQUE_NAME}_mimic_dctcp"
